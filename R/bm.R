...
#'
#' This function generatea a time series of one dimension fractional
#' Brownian motion.
#' adapted from
...
#' @param n the number of points between 0 and 1 that will
...
fbm <- function(hurst = 0.7, n = 100) {
  ...
  r[1] <- 1
  for (k in 1:n) {
    r[k + 1] <- 0.5 * ((k + 1)^(2 * hurst) -
                         2 * k^(2 * hurst) + (k - 1)^(2 * hurst))
  }
  r <- c(r, r[seq(length(r) - 1, 2)])
  lambda <- Re((fft(r)) / (2 * n))
  W <- stats::fft(sqrt(lambda) * (rnorm(2 * n) + rnorm(2 * n) * 1i))
